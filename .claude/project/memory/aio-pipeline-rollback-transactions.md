...
2 = reify the ctx with zero behaviour change; 3 = swap in `runGateSequence` + the undos). Its shape
is a **gate ARRAY**, not `runAioPipeline` — that takes `pre[] + ONE exe + post[]` and cannot express
13 mutations. Governing card: `TRDD-DQ6XN2VP`. [^6] [^7] [^8]

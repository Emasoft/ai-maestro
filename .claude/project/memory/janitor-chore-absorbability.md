...
^ATOM-2C2Q-R4RQ [desc: "BOTH writers stamp into ~/.claude/janitor-control/<chore>.last-run.ts (server janitorControlDir; janitor last_run_path → control_dir); the plugin-data global-state/*.last-run.ts copies are era-2 lefto", keywords: chore_stamp_frozen_since_July marketplace-refresh_last-run_stamp_never_updates which_directory_holds_the_janitor_chore_stamps global-state_last-run.ts_stale janitor-control_vs_global-state server_stamp_mtime_does_not_match_the_log_line is_our_server_writing_the_github-config-audit_stamp stampChoreRun_path janitorControlDir JANITOR_CONTROL_DIR_override two_stamp_directories janitor_says_a_chore_we_run_is_frozen read_last_run_max_over_eras last_run_path_control_dir, trdd: TRDD-X9VLHBFZ, ocd: 2026-09-05, lmd: 2026-09-05]
`stampChoreRun(chore)` (lib/janitor-chore-stamp.ts:173) writes `${janitorControlDir()}/<chore>.last-run.ts`, and `janitorControlDir()` (lib/janitor-control.ts:65) is `$JANITOR_CONTROL_DIR` or `~/.claude/janitor-control/`. The janitor 3.4.14 writes to the SAME dir: `global_state.last_run_path(task)` = `control_dir()/<task>.last-run.ts` (scripts/lib/global_state.py:300-310), and its `read_last_run` (:313-328) takes the MAX over control_dir and the older plugin-data `global-state/` path — so a correct janitor read cannot report a chore frozen while the control-dir stamp is fresh. Measured 2026-09-05: control-dir `github-config-audit.last-run.ts` = 1788595904 = 10:11:44+0200, identical to the second with the pm2 "[github-config-audit] 14 repos scanned" line; cache-prune 10:10:55 (its scheduler runs the first beat immediately at start, lib/cache-prune.ts `void beat(log)` before `setInterval`, and logs a run line ONLY when it removed something — a silent attempt still stamps, by the module's attempt-completion contract). The plugin-data `global-state/*.last-run.ts` files (github-config-audit 2026-07-21T20:36, marketplace-refresh 2026-07-21T20:23, cache-prune 2026-07-25, fleet-plugins-update absent) are era-2 leftovers no current writer targets — but the DIRECTORY is still live (daemon.heartbeat.ts, daemon.log, oauth-usage-cooldown.json written 2026-09-05), so do not call the dir dead, only those stamps. A hand `ls` of that dir reports every absorbed chore frozen since July; diagnose by `stat` of BOTH paths with full dates before believing either "dead" or "healthy". Reported to the janitor on ai-maestro#126 (2026-09-05); do NOT dual-write to make a stale reader happy.

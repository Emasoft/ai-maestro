...
^ATOM-6H30-LMB3 [desc: "where is the parked-card predicate now / the doctor and the store disagree about which card counts as parked / advanceColumn refused blocked but validate accepts the park / how do I add a new park for", keywords: parked_card_predicate_lives_once doctor_and_store_disagree_parked advanceColumn_refused_blocked_entry add_a_new_park_form hasOtherParkForm_gone frontmatterDayLocal_gone frontmatterDay_moved_to_trdd-vocabulary parkReason isParkedByOtherForm review-after_hub-blocked_fleet-ask per-form_neuter_one_doctor_one_store_case store_doctor_import_cycle, trdd: TRDD-4P798U6P, ocd: 2026-09-05, lmd: 2026-09-05]

The "is this card parked by a form other than column blocked / non-empty blocked-by" decision lives ONCE, in `lib/trdd-vocabulary.ts`: `parkReason(fm, todayDay)` returns `review-after` (future date, first), then `hub-blocked`, then `fleet-ask`, else null, and `isParkedByOtherForm` is its boolean view. Both consumers call it — the doctor's BLOCKED-WITHOUT-PROBE/BLOCKER-* rules in `lib/trdd-doctor.ts` (which also prints the reason it returned, never a re-derived one) and the store's `advanceColumn` entry gate into `blocked` in `lib/trdd-store.ts`. Until TRDD-4P798U6P (c9937cef, 2026-09-05) the store carried private byte-identical copies (`frontmatterDayLocal`, `hasOtherParkForm`) because the doctor imports FROM the store, so a back-import would have closed a cycle; the leaf vocabulary module is the home both already import. A fourth park form goes there and nowhere else; the proof that both consumers route through it is a per-form neuter (delete one `return` line) reddening exactly one doctor case and one store case — the label-form fixtures exist in both suites for that reason. `frontmatterDay` (day part of an ISO string or a Date) moved there too; the doctor no longer re-exports it.


^ATOM-CKNB-ES9P [desc: "the --min-severity test stays green after I break the filter / how do I pin a census that is now 0 rows / a test that only checks for empty output is vacuous / an unfiltered-has-N-lines positive contr", keywords: min-severity_test_vacuous test_still_green_after_breaking_the_filter census_pin_zero_rows 0_ERROR_rows_what_to_pin filter_test_passes_with_filter_broken empty_output_test_proves_nothing unfiltered_lines_positive_control_vacuous pin_exact_WARN_rows_toEqual cut_-f1-3_severity_rule_id error_empty_exit_0_pair neuter_filter_both_directions drop_everything_drop_nothing trddgrep_validate_WARN_floor_5_rows which_WARN_rows_remain STALE-COLUMN_979DBDAA BODY-STATE-CLAIM_v1_archived_cards re-pin_census_after_corpus_moves pillar-grep-cli_test_census, trdd: TRDD-MUB7NTRF, ocd: 2026-09-05, lmd: 2026-09-05]
A test that pins `trddgrep validate --min-severity error` to EMPTY output proves nothing once the ERROR census is 0: a filter that drops everything passes it too, and so does an "unfiltered run has >= N lines" control (it never exercises the filter). Pin the filter where the census is non-empty — `--min-severity warn` sliced to `severity\trule\tid` and compared with `toEqual` to the exact rows in the CLI's order (rule, then id) — and keep `error -> empty / exit 0` beside it: the pair covers both directions (drop-everything reds the WARN pin with `[""]`; drop-nothing reds the error half because the WARN rows print). Measured 2026-09-05 by neutering `scripts/trddgrep.mjs`'s filter both ways (`>=` -> `>`, predicate -> `true`): each reds exactly the census test (commit 3d317b3e, `tests/unit/pillar-grep-cli.test.ts`; no card of its own — the vacuity was exposed when TRDD-MUB7NTRF's superseding verb took the ERROR census 2 -> 0). The pinned rows at that commit: 4 FROZEN ones (BODY-STATE-CLAIM on the v1 UUID-named archived cards 70A521D9, 7123D51A, EAC02238, EF0C6C0A — permanent by rule 12) plus 1 LIVE one (STALE-COLUMN 979DBDAA, an `approval` card in tasks/ whose WARN disappears the moment it moves) — so the floor is 4 + 1, not a stable 5: when 979DBDAA moves, re-measure with `trddgrep validate --min-severity warn | cut -f1-3` and re-pin the list.
...
[^18]: [id: ATOM-LSBO-1MAU, status: valid, desc: "re-derived the hard way 2026-08-22: filed and retracted a false stale-trddgrep card because recall was never run", keywords: "trddgrep_validate_returns_0_hits_for_a_rule_I_just_added is_the_deployed_CLI_stale cmp_says_differs_so_the_install_is_old deployed_gate_cannot_see_a_new_rule launcher_vs_implementation filed_a_stale-binary_finding_then_retracted_it", ocd: 2026-08-22, lmd: 2026-08-22] DO NOT conclude the installed pillar CLI is STALE from `cmp` DIFFERS plus a zero-hit run, BECAUSE both signals are produced by a HEALTHY install: cmp compares a shared bash launcher against a per-tool `.mjs` (two different KINDS of file, 5428 vs 51370 bytes here), and a zero-hit run over an already-clean corpus is indistinguishable from a blind gate. DO seed one violation and re-run — if the rule fires, the gate sees it — and read the head of an unfamiliar executable before comparing it to anything. Measured 2026-08-22: this atom already said so and I filed a false card anyway, because I never ran recall.

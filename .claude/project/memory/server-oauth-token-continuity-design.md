...
directory, i.e. at the live cookie store. So the store is not in some other tree: it hangs off the
...
[^4]: [id: ATOM-TM7C-UJR3, status: valid, keywords: "grep_returned_nothing_so_the_path_is_unreferenced needle_is_a_joined_path path.join_by_segment include_ts_excludes_the_mjs_runtime how_to_prove_a_path_is_never_referenced", ocd: 2026-08-27, lmd: 2026-08-27] ⚠ SUPERSEDED 2026-08-27 by [^5] — its PRESCRIPTION is itself incomplete (the needle it recommends omits `profiles`, the very segment whose absence it asserts, and `rotatorRoot`, the accessor that RETURNS the legacy root), and the whole approach was later superseded again by resolving the filesystem instead of grepping. Kept as the dated guardrail, not as advice. ORIGINAL TEXT FOLLOWS. DO NOT prove "this codebase never touches path X" with a grep for a JOINED path literal (`account-rotator/profiles`) restricted to `--include=*.ts`, BECAUSE this repo builds paths by SEGMENT through helpers like `legacyRotatorRoot()` — so no file would contain that substring even if a caller joined `profiles` onto it — and `*.ts` excludes `server.mjs` / `lib/*.mjs`, which CLAUDE.md says are loaded at runtime and go live on a restart with no build, plus `scripts/`, `components/` and `.claude/scripts/`. The needle searched for the one shape the code is least likely to have, over the half that is easiest to change. DO search SEGMENTS and the HELPER NAME across every file type instead, with a positive control that must hit the helper's definition: `grep -rnE "account-rotator|chrome-profile|legacyRotatorRoot|Cookies" --exclude-dir=node_modules --exclude-dir=.git .` — if it does not return `lib/oauth-rotator/slots.ts`'s definition, the command is broken rather than the codebase clean. Run that way the answer held (17 hits, zero `profiles`/`chrome-profile`/`Cookies`), so the atom's claim stands — but it stood on evidence that could not have contradicted it.
[^5]: [id: ATOM-D4TM-YGF8, status: valid, keywords: "prove_a_path_is_never_referenced needle_omits_the_segment_it_asserts_absent path_join_root_seg_invisible_to_a_joined_literal_grep a_cap_on_a_command_settling_a_negative enumerate_what_is_joined_onto_the_root", ocd: 2026-08-27, lmd: 2026-08-27] DO NOT prove "this codebase never touches path X" with a needle that omits X's own SEGMENT or the accessor that RETURNS the root, BECAUSE a `path.join(root(), 'seg')` form matches neither. My first needle searched a joined literal in *.ts; my CORRECTION searched `account-rotator|chrome-profile|legacyRotatorRoot|Cookies` — still missing `profiles` (so "zero profiles hits" was asserted about a term the instrument never carried) and missing `rotatorRoot`, the function that RETURNS the legacy root when the opt-in is set, so it bounded the callers of the wrong function. The same file demonstrably uses the blind form: `path.join(rotatorRoot(), 'live-identity.json')`. DO enumerate what is actually joined onto the root instead: `grep -rnE "rotatorRoot|profiles|chrome-profile|Cookies|account-rotator" --exclude-dir=node_modules --exclude-dir=.git .` with NO `head` (a cap on a command settling a NEGATIVE is you choosing the absence), plus `grep -rnE "join\((root|r|dir)[, )]" lib/oauth-rotator/*.ts` for the indirect form. Run that way: 80 hits (not 17), and the CLOSED set joined onto a rotator root is state.json, slots/, live-identity.json, active-alerts.json, opt-in.flag, cookie-leg-since.json, tick-completed.ts, rotator.log — no `profiles`, and zero `Cookies`/`chrome-profile` in the tree. The claim holds; three successive needles did not.

...
### The guard now REFUSES when it cannot resolve a project root (fixed 2026-08-04)

It used to `exit 0` there — allow **every** write, anywhere — printing a warning to a stderr
stream nothing reads. Found by driving `.claude/scripts/subagent-write-guard.sh` directly under
CC 2.1.221, which also confirmed it discriminates correctly when the root IS resolvable (outside
path → exit **2 BLOCKED**, inside path → exit **0**).

Resolution order is now `CLAUDE_PROJECT_DIR` → the hook payload's `.cwd` → `git rev-parse
--show-toplevel` → **refuse with exit 2**, naming all three attempts. Fixed under
**TRDD-YR4G2CZH**; four cases in `.claude/scripts/test-subagent-write-guard.sh` pin it, and that
harness — which nothing had ever run — is now executed by the suite via
`tests/unit/subagent-write-guard.test.ts`.

Failing closed was measured to cost nothing on the hook path *before* it was chosen: all four
agents below build the hook command from `${CLAUDE_PROJECT_DIR}`, so with the variable unset the
hook's own path does not resolve and the script never runs. What the fix actually buys is the
other case — Claude Code expanding that placeholder into the command PATH without exporting the
variable into the hook's ENVIRONMENT, where the script runs normally, sees an unset var, and (in
the old form) went silently inert for every write of the session.

### Still open — that Claude Code INVOKES the hook at all

Unproven under 2.1.218's workspace-trust requirement, and *"the guard went inert"* is
indistinguishable from *"the guard allowed the write"* from the outside, so no config read
answers it. Proving it needs a real agent spawn attempting a write outside the project and
outside `/tmp` — see TRDD-9X2STNL2's open acceptance box. Spawn the agent by its **BARE name**: a

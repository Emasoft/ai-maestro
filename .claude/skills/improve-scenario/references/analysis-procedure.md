...
Then collect every report related to this scenario by globbing:

```
${CLAUDE_PROJECT_DIR}/tests/scenarios/reports/SCEN-<padded-id>_*.report.md
${CLAUDE_PROJECT_DIR}/tests/scenarios/reports/scenario_proposed-improvements_<padded-id>_*.md
```

Sort all matches by timestamp (descending — newest first). If zero reports exist, tell the user: "SCEN-<padded-id> has no run history yet. Run it at least once via `run-scenarios-batch <N>` before asking for improvements." Stop.
...
1. **Flaky step** — same step passes on some runs and fails on others. Usually a missing `wait_for`, race condition, or timing-sensitive verification.
...
6. **Wrong cleanup order** — tmux sessions or agent folders leak because cleanup deletes resources in the wrong order. Reorder the cleanup phase.
...
1. **Pattern** — short name for the issue (e.g. "Flaky S014: missing wait_for after task send")
...
Do NOT edit the scenario file from this skill. The user's `edit-scenario` skill is the authoritative path for applying edits. Your output is a proposal document.
...
```
${CLAUDE_PROJECT_DIR}/tests/scenarios/reports/scenario-improvement-analysis_<padded-id>_<timestamp>.md
```
...
<Which proposals to apply first; reference `edit-scenario` as the tool to apply them.>
```

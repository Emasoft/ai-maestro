---
name: scenario-region-capture
description: >-
  Token-cheap visual verification for UI scenarios. Capture DOM/ARIA-scoped
  CLIPPED screenshots (target element + margin) and SCOPED aria snapshots
  instead of full-page captures — using the live ARIA/DOM tree as the guide for
  what to clip. Use whenever a scenario step must visually check CSS/layout
  (truncated text, misaligned elements, wrong gradient, missing highlight/focus
  ring, elements bleeding out of their container, obstructing overlays).
  Preloaded via the scenario-runner skills frontmatter.
disable-model-invocation: true
---

# Scenario Region Capture — DOM/ARIA-guided, token-cheap visual checks
...
240–367-turn run they dominate cost (TRDD-N1FYP2AW). The fix is NOT to stop
verifying — CSS bugs are invisible to text and genuinely need pixels. The fix is
to **observe only the region under test**, located via the DOM/ARIA tree. Image
tokens scale with area `(w·h)/750`, so clipping collapses the cost:
...
named "Delete agent"; the `navigation`), resolve it to a bounding box from the
live DOM, clip to it. Never guess pixel coordinates; never capture the whole page
unless the check is truly global.
...
3. **Is the check genuinely page-wide?** (cross-viewport stacking, modal
   obscuring everything) → `captureLandmarks()` — N small per-landmark clips, not
   one big page. A bare full-page `page.screenshot()` is the last resort.
...
- `await scopedAria(page, target)` → a compact ARIA text tree of just that
  subtree. The DEFAULT for verification.
...
It uses only portable primitives (`page.evaluate`, `page.screenshot({clip})`)
with a try/catch fast-path for `locator.ariaSnapshot()`, so it works regardless
of which Playwright surface the dev-browser build exposes.

## Integration with the scenario runner

- This **replaces** a bare `aim_screenshot` / `page.screenshot({fullPage:false})`
  for any **verification** capture (the current `aim_screenshot` grabs the whole
  ~1,365-token viewport with no element clipping).
- Default to `scopedAria()`; reach for `captureRegion()` only for genuinely
  pixel-level checks.
- A clipped image is the right input for the Opus `screenshot-interpreter` agent
  — send it the ~40–320-token clip + one focused question, not a full page.
- Save clips under the Rule-10 per-run dir
...
- `locator.ariaSnapshot()` / `getByRole()` are newer; the helper only uses them
  on a try/catch fast path and falls back to `page.evaluate`, so nothing breaks.
- If `page.viewportSize()` is absent the helper defaults to 1280×800 — override
  for `device: tablet`/`smartphone` scenarios.

...
- **Timestamp**: 2026-03-22T21:09:50.957Z
...
Context: Next.js 14 App Router TypeScript project (AI Maestro dashboard). BuildAction.tsx handles UI for building a plugin — shows build progress, status polling with setInterval/clearInterval, and results display. Recently fixed: error display condition changed from `error && !result` to `error` so errors show even when result holds last known status.

...
**Governance-password rotation — `POST /api/governance/password/invalidate` (TRDD-P7XKV3N9).** The human revokes the password by supplying it (possession) plus a one-shot code delivered to the physical desktop via the OS notification channel (presence, reusing `lib/setup-bootstrap.ts`); the hash is then DESTROYED (`invalidatePassword()`), so the next login demands a new one. Console-gated (`isConsolePeer`), throttled per-peer, fail-closed (no notification channel ⇒ refuse). **This line used to read "this op + MAESTRO login ONLY, never other routes" — that was false in both directions and is corrected here (2026-08-02):** MAESTRO login is *not* console-gated (see "Still open" below), and four other callers already existed. The authoritative, categorised caller census lives in `lib/peer-address.mjs`'s own docstring — read it there, do not re-state it here, because a second copy is what drifted. It separates **presence as a second factor** (the R48 ops: password `invalidate`/`reset`, `lib/oauth-rotator/reauth-guard`, `app/api/settings/edit`) from an **origin check on a local-only fact** (`app/api/statusline/ingest` — a statusline payload describes a process on THIS machine, so a remote caller has nothing truthful to say; it confers no capability and returns no secret). The list is valuable because it is SHORT and every entry argued for itself; adding one is a deliberate ruling, not a convenience. Two surfaces call it and carry no policy: Settings → **Revoke** and `aimaestro-governance.sh invalidate-password` (TTY prompt, never argv — a password on argv leaks via `ps`/history, TRDD-E9BZ5P7S). Still open: MAESTRO *login* is not yet console-gated, and the general TTY→sudo-token path for other strict routes is TRDD-9MZQ4T7E.

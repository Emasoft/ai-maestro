...
> **3-role finalization (TRDD-H18PO5YJ).** This build EXPOSES only **3** titles for NEW-agent selection — **MANAGER** (single governance authority), **MAINTAINER** (imported-plugin agents), **AUTONOMOUS** (existing agents) — via the SSOT `ACTIVE_GOVERNANCE_TITLES` in `types/agent.ts`. The other 6 (COS / ARCHITECT / ORCHESTRATOR / INTEGRATOR / MEMBER / ASSISTANT) stay **dormant**: the full `AgentRole`/`GovernanceTitle` union, `VALID_GOVERNANCE_TITLES`, `TITLE_PLUGIN_MAP`, `PLUGIN_COMPATIBLE_TITLES`, the comm graph, and the role-plugin repos are ALL intact — so existing dormant-title agents stay valid and resolvable. Only the **wizard title picker** and the **title-assignment dialog** filter their OFFERED titles through `ACTIVE_GOVERNANCE_TITLES` (the dialog also always keeps the agent's CURRENT title visible so a dormant-title agent can switch away). Reverting to the full model = restore that one constant to `VALID_GOVERNANCE_TITLES`.
...
**The perimeter is the network; the gate is identity (not network position).** Inside the Tailscale VPN, *every* device is a first-class caller — a phone, an iPad, a remote machine, a script on a server — because every function must work from a browser when working remotely. Reachable ≠ permitted: each route still demands its own proof (a session cookie for a human, an AID proof for an agent, a sudo token for a strict route). Do NOT gate a route on "where the packet came from" — that breaks remote administration, and it isn't a boundary anyway (see the trusted-peer note below). Every route is curl-able, so a check placed in a *client* (button, CLI, hook) is skippable with one curl — it is not a weak check, it is no check. Enforce in the route.

**The trusted client IP — `lib/peer-address.mjs` (TRDD-P7XKV3N9).** A route handler gets a `Request`, never a socket, so it can only learn the caller's address from a header — and `x-forwarded-for` / `x-real-ip` are **client-forgeable** (a phone sends `X-Forwarded-For: 127.0.0.1`). So `server.mjs` **deletes any inbound `x-aim-peer` and re-stamps it from `req.socket.remoteAddress`** (the one place that sees the real TCP peer; we sit behind no proxy), and `lib/peer-address.mjs` is the only sanctioned reader. `isConsolePeer()` accepts `::ffff:127.0.0.1` too — the `::` bind is dual-stack. NEVER read `x-forwarded-for` for a security decision. Verify any spoof test from a genuinely *remote* peer (the host's own Tailscale IP) — from loopback the peer really is 127.0.0.1 and the test proves nothing.

**Governance-password rotation — `POST /api/governance/password/invalidate` (TRDD-P7XKV3N9).** The human revokes the password by supplying it (possession) plus a one-shot code delivered to the physical desktop via the OS notification channel (presence, reusing `lib/setup-bootstrap.ts`); the hash is then DESTROYED (`invalidatePassword()`), so the next login demands a new one. Console-gated (`isConsolePeer` — this op + MAESTRO login ONLY, never other routes), throttled per-peer, fail-closed (no notification channel ⇒ refuse). Two surfaces call it and carry no policy: Settings → **Revoke** and `aimaestro-governance.sh invalidate-password` (TTY prompt, never argv — a password on argv leaks via `ps`/history, TRDD-E9BZ5P7S). Still open: MAESTRO *login* is not yet console-gated, and the general TTY→sudo-token path for other strict routes is TRDD-9MZQ4T7E.
...
- **[docs/CLAUDE-CODE-COMPATIBILITY-AUDIT.md](./docs/CLAUDE-CODE-COMPATIBILITY-AUDIT.md)** - Per-version verdict for Claude Code 2.1.113-2.1.209 changelog entries (six passes); lists repo-by-repo follow-up for items that affect role-plugins / ai-maestro-plugin separately

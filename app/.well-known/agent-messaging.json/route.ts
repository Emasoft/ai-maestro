...
export async function GET(_request: NextRequest): Promise<NextResponse<WellKnownResponse | { error: string }>> {
  try {
    ...
    // Guard against undefined values that would produce "undefined.example.com"
    if (!selfHostId || !providerDomain) {
    ...
    console.error('[AMP WellKnown] GET error:', error)
    return NextResponse.json(

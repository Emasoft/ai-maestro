          ...
          filtered.map((mkt) => {
            ...
            return (
              <div key={mkt.name} className="rounded-lg border border-gray-700/30 bg-gray-800/20 overflow-hidden">
                {/* UI2-MAJ-06: keyboard-accessible expand/collapse for each marketplace row */}
                <div
                  role="button"
                  tabIndex={0}
                  aria-expanded={isExpanded}
                  aria-label={`${isExpanded ? 'Collapse' : 'Expand'} ${mkt.name}`}
                  onClick={() => setExpandedMkt(isExpanded ? null : mkt.name)}
                  onKeyDown={(e) => {
                    if ((e.key === 'Enter' || e.key === ' ') && e.target === e.currentTarget) {
                      e.preventDefault()
                      setExpandedMkt(isExpanded ? null : mkt.name)
                    }
                  }}
                  className="flex items-center gap-2 px-2.5 py-2 cursor-pointer hover:bg-gray-800/30 transition-colors focus:outline-none focus:bg-gray-800/40"
                >
                  {isExpanded

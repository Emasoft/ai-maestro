...
updated: 2026-09-09T12:26:23+0200
...
## ⏵ STATE — READ THIS FIRST ON RESUME (authoritative; supersedes the body) — 2026-09-09 (CLOSED)

**CLOSED 2026-09-09: parts 1, 2, the part-2 residue and 3 all landed (dated bullets + log below). History as of 2026-08-29 — Part 1 was DONE for the READ helper only. Two of this card's premises were wrong; read
...
**NEXT ACTION — NONE (closed 2026-09-09: part 3 landed 2026-09-05 as 4cabbee7, residue as 2d919cb3 — see the log lines below; the struck paragraph is the 2026-08-29 question, kept as history).** ~~Decide where part 3 lives now that it is a rollback-integrity finding
...
- [x] ~~Factor the `ensure`/`update` trios into one parameterized helper set.~~ **DECLINED as specified — box ticked 2026-09-09 to close the checklist as RESOLVED-BY-DECLINE, not as done** — measured, they differ on `claude` handling, not just on a name string; see STATE. Reopen only with a design that keeps the two behaviours distinct.
...
- 2026-09-05T17:28:48+0200 — CORRECTIONS FORWARD to 2d919cb3's commit message (not amendable): (1) 'each restored with an empty diff' is MEASURED for the verifier's two neuters (byte-identical against cp backups, verify report sections 4-5) and SECOND-HAND for the implementer's two — its route neuter is proven only by the neuter text's absence from the diff, its owner neuter's diff reference is unstated in its report; (2) 'the only other DeleteMarketplace caller is startup cleanup through the service' was measured over server.mjs and lib/*.mjs only. Provenance note: the uncited auto-update commits 71b9f796 (+2 key lines) and 793b866c (+7) in services/auto-update-service.ts are writer 6's origin (the PE54D95Q lane), later repointed by Part 3 — other work, not this card's implementation; 5438312f and 7674ac60 touch the key nowhere. 3e40eaa8's commit time is 2026-09-05 10:07, so its position in implementation-commits is by clock.
- 2026-09-09T12:26:22+0200 — column → testing by governance-rules-session. full suite last green at part 2; parts 3 and the part-2 residue carry their own suites (10/10, 13/13) plus an independent verifier on 2026-09-05 — cited as the testing/ai_review evidence this hop records
- 2026-09-09T12:26:22+0200 — column → ai_review by governance-rules-session. full suite last green at part 2; parts 3 and the part-2 residue carry their own suites (10/10, 13/13) plus an independent verifier on 2026-09-05 — cited as the testing/ai_review evidence this hop records
- 2026-09-09T12:26:23+0200 — COMPLETE by governance-rules-session. parts 1-3 and the part-2 residue landed and verified (implementation-commits); the ensure/update merge box declined as specified.

...
updated: 2026-09-05T15:59:40+0200
...
- 2026-09-05T15:31:57+0200 — ownership CORRECTED by measurement (w-own4.txt) — this line SUPERSEDES the 15:17, 15:24 and 15:27 ownership statements: ~/.claude/statusline.py is INSTALLED by the llm-externalizer plugin — its scripts/statusline/install.sh (:4, :34) and scripts/install_statusline.py (:9, :16) copy scripts/statusline/statusline.py to that path with chmod 755, and every cached release (13.5.10, 13.5.11, 13.6.0) carries GIT_TIMEOUT = 1.0 (:108) and the git -C <cwd> status --porcelain=v1 call under it (:125/:128) — grepped individually. The installed copy differs from all three by 45 diff lines (three hunks after line 464; 858 vs 841 lines) but its git block (lines 98-132) is byte-identical to the shipped 13.6.0, so the defect is in the shipped source. Route: Method 1 — filed as Emasoft/llm-externalizer-plugin#14 (the repo named in the plugin's plugin.json; 'Emasoft/llm-externalizer' does not exist); the installed copy needs the same one-flag patch until a fixed release re-installs it — the USER's call on their own file. AgentlensPro's 'the user's personal' wording (statuslineUsage.ts:157) described location and non-ownership by AgentlensPro only.
- 2026-09-05T15:35:10+0200 — wording: 'the USER's call on their own file' in the previous line means the INSTALLED COPY in ~/.claude is theirs to patch locally; authorship of the script is the plugin's — the two statements do not conflict. Measurement scope: :108 and :125 were grepped in all three cached releases, :128 in 13.6.0 only. The 45 diff lines are 29 added after plugin line 464 plus two 7→1 replacements at plugin lines 813-819 and 823-829 (net +17); whether they are a later release, the installer's post-copy patch, or local edits is not determined. Issue #14's body was edited to say the same.
- 2026-09-05T15:36:48+0200 — correction to the previous line: 'the installer's post-copy patch step' is NOT a candidate for the 45 differing lines — install.sh (13.6.0) copies statusline.py verbatim (cp -p SRC DEST after backing up the old copy; skips when cmp says identical) and the 'patches' in its header refers to ~/.claude/settings.json (statusLine command + refreshInterval 3), read from lines 1-11 and 69-75. Remaining candidates: a later release than the three cached, or local edits. Note: a re-run of install.sh would OVERWRITE the installed copy (with a timestamped backup), so a local --no-optional-locks patch survives only until the next install. Issue #14 edited accordingly.
- 2026-09-05T15:39:29+0200 — install.sh read to its end (150 lines): after the copy the only write to the installed script is chmod +x; the settings.json step is a python heredoc that uses the path as a string. 'Copies verbatim' therefore holds for the whole installer, not only the copy line. Mtimes (cp -p preserves the source's): cached 13.6.0 statusline.py 2026-08-26 04:47:24, installed copy 2026-08-29 15:48:20 — three days later than the cached source, so the installed copy was either edited after install or copied from a release newer than the three cached; still not determined, but the 'Aug 29 mtime' cited on 15:27 as an ownership signal is an edit-or-newer-release time, not an authorship signal.
- 2026-09-05T15:41:29+0200 — scope of the previous line made good: lines 76-150 of install.sh were then printed IN FULL (the previous line's 'read to its end' had rested on an awk filter). Confirmed by reading: after either cp -p branch the only operation on the installed script is chmod +x; the python heredoc writes only settings.json (tmp file + os.replace) and uses the script path solely to build statusLine.command. So 'the installed script is never rewritten by the installer' is now MEASURED. Refinement: 'copied from a release newer than the three cached' should read 'copied from a source file with a later mtime — a newer release, or a fresh checkout of the same one'; the 45-line CONTENT diff, not the mtime, is what points at a different version. Thread closed on this card; the fix lives in Emasoft/llm-externalizer-plugin#14.
- 2026-09-05T15:59:38+0200 — the owed upstream issue is FILED: Emasoft/claude-plugins-validation#230 (cpv_pre_install_scan.py's uv-run child dies on ModuleNotFoundError: yaml in 5.17.0; exit 2 propagated by the pre-install-scan wrapper as no-verdict). Duplicate search first: #14 (closed — pyyaml import error for validate_skill/validate_security on bare skill folders) and #70 (closed — cpv-pre-install-scan unusable, --json wrote repo lint to stdout) are related by TITLE, bodies not read. Surface numbering confirmed: surface 1 = aimaestro-cpv-pre-install-scan.sh (the exit-2 one), surface 2 = aimaestro-cpv-validate-plugin.sh (exit 0 via remote_validation.py) — the card's earlier lines had the numbers right.

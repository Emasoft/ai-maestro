...
- 2026-08-28T23:13:18+0200 — review-fork settle 2: `dueForDelivery(rec, nowS)` (alert-delivery.ts:83) gates on `lastDeliveredAt` / `seen` / `firstSeenAt` only — no operand derives from `message`; the stored record just overwrites `message` each beat. So a per-beat-changing account list rewrites the stored text and never re-arms backoff. Read from the function body, not a keyword grep.

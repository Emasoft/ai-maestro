...
- 2026-08-28T23:01:20+0200 — second review-fork settle, HARNESS fact not model self-report: in a FRESH empty dir (no settings.local.json), `-p` asked to CALL the Skill tool on `ai-maestro-janitor:janitor-findings`. Normal: the skill loaded and stopped at its own `uv run` permission gate (a real invocation). `--restricted`: the Skill tool returned `Unknown skill: ai-maestro-janitor:janitor-findings`. Same runs: `Found 77 plugins` vs `Found 0 plugins`. Also closes the earlier janitor NO-SKILL attribution (scratch-dir local disable) — the janitor skill is reachable in a clean dir. Note: both earlier runs had hooks-modules rollout OFF, so the 20-vs-0 hook count is a legacy-path measurement.

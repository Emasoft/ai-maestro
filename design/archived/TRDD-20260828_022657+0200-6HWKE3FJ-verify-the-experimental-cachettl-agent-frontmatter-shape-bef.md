...
- 2026-08-28T23:29:54+0200 — positive control for the "0 parse errors" reads: `model: not-a-real-model` + `description: probe-DESC-MARK` + `cacheTtl: 42` → the agent is offered, BOTH marker strings are echoed verbatim from the Agent-tool description, 0 errors. The description build validates no field at all (the `has invalid maxTurns`/`permissionMode` errors fire at spawn, not at load), so "tolerates every shape" is a whole-file property, not `experimental`-specific. Verdict unchanged: code-level evidence only.

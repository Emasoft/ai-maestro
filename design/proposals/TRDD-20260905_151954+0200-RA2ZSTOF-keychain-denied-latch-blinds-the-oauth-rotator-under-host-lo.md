...
updated: 2026-09-05T15:32:07+0200
...
- [ ] a Tier-0 implementation card minted for the accepted items, citing this card, with the latch threshold and the attribute-only exemption PREDICATE (argv-based, the janitor's wording: find-generic-password or list-keychains WITHOUT -w cannot prompt and never latch on a timeout; anything carrying -w, and every add-/delete-generic-password, can prompt and counts) stated as ONE shared contract with the janitor's python twin — a config FILE for the constants is this card's decision, not the janitor's
...
- 2026-09-05T15:26:51+0200 — box 2 audit trail: it was "the latch threshold and exemption list stated as ONE shared contract…" at mint; reworded 15:24 to name the attribute-only exemption PREDICATE and to assign the config-file question to this card; reworded again now to carry the janitor's exact predicate wording (can-prompt is the reason, counts is the consequence; 'anything carrying -w', not only -w reads).
- 2026-09-05T15:26:51+0200 — janitor session's self-corrections (third message, data): (a) 'under host load' as the CAUSE of the latch is WITHDRAWN — what was measured is only that the security calls STALLED; the server's own TmuxServerKeychainWatchdog logged keychain_probe_timeout eight times between 13:59:32 and 15:08:55 (pm2-error), a second instrument seeing security stall machine-wide from 31 min before the 14:30:36 latch; at loadavg 11 (15:22) five not-found attribute reads took 0.01-0.02 s; CPU load vs securityd contention (watchdog probes + slot reads + hooks + statusline) vs something else is NOT established — the three asks stand as POLICY (a stall is not a denial), not as a cause. (b) 'the python burn gate would have projected the wall at ~14:28' is WITHDRAWN — its ring was empty (yielded beat) and the estimator was never replayed; the last two samples (80→82% over 5 min) show the slope halving, which at 82% puts the wall ~45 min out, outside ROTATE_HORIZON_MIN=15; the checkable statement is only that tick.ts has no projection and 97% leaves 3-20 min at that day's slopes. (c) rotator.log 15:01/15:02 'cascade: renew-cookie=…' lines are a latch artifact (all-False AccountState), not two accounts needing cookies. The Problem section's loadavg figures stay as measurements; the Root cause's 'under host load' and item 3's 14:28 projection are superseded by this line.
- 2026-09-05T15:31:57+0200 — janitor session's fourth message (data, their final review corrections): (1) '55 beats/hour' was a decision-LINE count (a beat can emit several lines); the measured statement is: no gap over 5 minutes between decision lines 09:46→14:16. (2) Their '~45 min out, outside ROTATE_HORIZON_MIN' is WITHDRAWN as a counterfactual of the same class as the 14:28 one; interval slopes were noisy, not trending — 14:20→14:24 1.4 %/min, 14:24:22→14:25:29 3.6 %/min (76→80 in 67 s), 14:25→14:30 0.4 %/min — so at those slopes the 97% trigger fires 1-7.5 min before the wall; neither gate's estimator has been run on them. (3) The 'two instruments' claim is checked: pm2 (God Daemon 10551 ← launchd) and the tmux server (9169 ← launchd) are separate process trees, so the server watchdog's keychain_probe_timeout lines are an independent observer of the same 13:59-15:08 window — 'machine-wide' is one step too far (two trees, not all); cause unknown. (4) If this card's Root cause names load as the cause it inherits the withdrawn claim (it does, in prose — superseded by the 15:26 line); the fix's scope is narrower than first written: it removes the false positive for TRANSIENT stalls and, for a persistently blocked keychain, only delays the latch by two reads — there the latch is right.

...
blocker-probe: gh issue view 116 --repo Emasoft/ai-maestro --json state -q .state
...
- 2026-08-28T23:48:13+0200 — review-fork finding FIXED in the same session: the sed range failed OPEN (START anchor broken → `verbs=0 fingerprint=e3b0c44298fc`, sha256 of nothing, exit 0; END anchor broken → range runs to EOF, hash file-wide). `_dispatch_table` now requires a non-empty capture containing the closing arm and returns 2; `--version`/`--capabilities` compute first and `|| return 2` (a `$(…)` inside an echo cannot fail the echo — measured: the guard alone still exited 0). Two `it.each` anchor-break tests on a temp copy of scripts/; neuter (guard removed) reds exactly those 2 of 66; 81/81 green with the manifest check; tsc 0; shellcheck quiet. §6.5 states the fail-closed property and that in-`dispatch()` comments move the hash.
- 2026-08-28T23:50:10+0200 — review settle: per-predicate neuter — dropping ONLY the closing-arm check (`grep -q` on the capture) reds exactly the END-anchor test (1 of 66), so the two guard predicates are pinned independently (START by `-z`, END by the arm check). The END mutant is syntactically valid (`bash -n`) and fails from the guard (`dispatch table not found`, exit 2), not from a syntax change. `shellcheck -S info`: SC2015 count 0 (resolved, not demoted).
- 2026-08-28T23:53:15+0200 — probe note: `gh issue view 116 … --json state -q .state` runs live here (→ OPEN). But NOTHING executes `blocker-probe` fields today — grep for `blocker-holds-if|blocker-probe` over scripts/, lib/ (excluding the doctor) and the installed janitor plugin finds no runner; the doctor validates the field SHAPE only (`BLOCKED-WITHOUT-PROBE`, `-BAD-PREDICATE`, `-NO-CANARY`). Pre-existing, corpus-wide (GFX57106 has the same three fields); recorded here, not fixed here.

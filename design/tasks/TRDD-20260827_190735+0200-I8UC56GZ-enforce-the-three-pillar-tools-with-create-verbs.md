...
- [x] Same for `prrdgrep` (a rule) and `specgrep` (a clause/spec), or a recorded reason why the
      shape differs
- [x] Every write path (`new`, `edit`, `fix`) lints AFTER the write and reports non-autofixable
...
- [x] `META-MISSING` reaches 0 on the corpus, or each remaining case is explained in place
...
- [x] **`split` / `supersede` / `merge`** — the derived-TRDD operations, which today are pure
...
- [x] **The TRDD pre-write gate is a NO-OP today — make it real.** `pillarPreWriteCheck` early-
      ...
      this box is done
- [x] **`fix` (the autofixer) is never invoked automatically by any write path** — the USER's
...
- [x] **`prrdgrep add` — BUILT, then REVERTED, and the spec it was built from was WRONG in two ways only the real document could show.** The spec (written from a toy fixture): mint max-number+1, version .1, insert after the last rule. Measured against the live PRRD: (1) **rules are MULTI-LINE** — G12.1 spans lines 146-172 with a BLANK LINE at 161 inside it, so "the line after the declaration" is INSIDE the previous rule's body and neither "next line" nor "stop at the first blank" finds its end; (2) **the tiers are SEPARATE SECTIONS** (GOLDEN, then `---`, then `## SILVER rules` at 176), so appending after the last rule of EITHER tier puts a silver rule in the golden section. Each would corrupt the project's constitution — the exact failure G12.1 exists to prevent, committed by the tool built to enforce it. Also learned by walking into it: `replaceAtLines` treats a multi-line `replace` as ONE line's content, so anchoring on a declaration hands the gate a line that keeps its id and changes its text, and it correctly refuses — the trap is real and the verb hit it. CORRECTED SPEC: the insertion point is the end of the TIER'S OWN section (before its trailing `---`), found by section heading, never by proximity to a rule; a rule's block ends at the next section divider, not the next blank. The 80%-correct implementation (id minting, the `--user` gate that keeps GOLDEN user-only, the specgrep refusal) is in `git stash@{0}`, not thrown away.
- [x] **The corruption is now MEASURED, and the corrected spec above was itself wrong.** (1) MEASURED: ran the stashed verb against a COPY of the live PRRD. It SUCCEEDED and inserted `- **S13.1**` at line **147** — inside G12.1's multi-line body, in the **GOLDEN** section, for a **silver** rule. Both predicted corruptions, demonstrated rather than inferred. The sharper half: it FAILED on my toy fixture (empty `expect` on the trailing newline) and SUCCEEDED on the real document — **the fixture was hiding the bug by failing for an unrelated reason.** Had the fixture carried one non-empty following line, this would have shipped. (2) MEASURED, on a rule that is NOT last: mid-section rules are separated by a BARE BLANK LINE followed by a `- **` declaration (G7.1/G8.1/G9.1/G10.1/G11.1 at 108/116/121/135/140, blanks at 107/115/120/134/139). So "a rule's block ends at the next section divider" — my own correction one entry above — is wrong for every rule except the LAST, and would swallow every following rule in the section. I generalised a terminator from G12.1, whose block happens to end at a divider because it is last. RE-CORRECTED — **RECORD OF A WRONG GENERALISATION, NOT A SPEC** (the shipped verb uses NONE of it; a predicate sitting in an acceptance entry reads as authority regardless of the prose around it, which is this card's own recurring failure): a rule's block ends at a blank line FOLLOWED BY a declaration, or at a section divider, whichever comes first — and note the internal blank at 161 does not terminate G12.1 because 162 is prose, not a declaration, so the predicate is consistent with both shapes. The insertion strategy does not need the terminator at all: append at the end of the TIER'S OWN section.
- [x] **Two audit corrections on the entry above.** (1) **The corruption claim was ONE measurement cited as three.** The diff shows `146a147` and nothing else; "inside G12.1's body", "in the GOLDEN section" and "a SILVER rule" are three READINGS of the same earlier awk. The conclusion holds — 147 sits between the G12.1 declaration at 146 and the `---` at 174, and `sed` confirms 147 is an indented continuation — but the evidence is one measurement, not three. (2) **The tree was verified with my own filter, not with git.** After `git stash apply` I checked `grep -c "case 'add'"` on one file and a `git status` filtered to drop a whole directory; an apply also restores the INDEX, which neither can see, so every other number in that commit was taken over an unverified tree. Asked git instead: `git status --porcelain` = **42 modified, ALL under `.claude/project/memory/`, +1 line each** (the owner-sanctioned librarian churn), **zero** paths outside it, and `git diff --stat HEAD -- lib scripts tests` empty. The apply left nothing; the numbers stand — but they stand because of this check, not the earlier one. (3) The re-corrected terminator is now measured OUTSIDE its original window too: G1.1/G2.1/G3.1-G6.1 at 29/36/75/77/80/95, each preceded by a bare blank, and G2.1's internal blanks at 42 and 67 are followed by prose rather than declarations — so the predicate holds document-wide, not just in 100-176.
- [x] **`prrdgrep add` SHIPPED, anchored on the SECTION rather than on a rule.** `prrdgrep add golden|silver "<text>"` mints the next number across BOTH tiers (never reused, never a gap-filler), version .1, and inserts at the END OF THE TIER'S OWN SECTION — located by heading, bounded by the trailing `---` or the next `## `. That boundary line is also the anchor and is never a declaration, which is what keeps the gate's version-bump clause out of the way. GOLDEN requires `--user`: an agent must not be able to mint a golden rule, and offline that is the honest stand-in for the `$AID_AUTH` check `prrd-edit.py` does against the server. specgrep gets a REFUSAL with a reason instead of a verb — a spec id encodes a family the author chooses, so there is no next id to mint. VERIFIED AGAINST A COPY OF THE LIVE PRRD, not only a fixture: G14.1 landed at 174 (after G12.1's body, before the `---`), S13.1 at 183 (inside the SILVER section), lint clean at 14 records. The test fixture is built from the SHAPES MEASURED on the real document (a multi-line rule with an internal blank; two tier sections) and carries a DIFFERENTIAL asserting the real PRRD still has them — because a fixture that has quietly stopped resembling its subject is this card's own recurring failure. Neuters: ignore the tier → the silver-placement test reds; restore the old 'after the last rule' anchor → BOTH placement tests red; drop the `--user` gate → the golden-refusal test reds.
- [x] **`split`/`merge` — CLOSED BY MEASUREMENT, not built.** `merge` is already expressible: superseding N cards with one is `trddgrep move <id> superseded --superseded-by <new>` per card, measured end to end earlier on this card (column, zone move and `superseded-by:` all written). `split` would write the derived back-link on BOTH ends atomically — and the invariant it protects currently holds with **ZERO violations across 101 relationships**: 101 cards carry `derived: true`, parents name 103 ids in `npt:`/`eht:`, and every derived card is named by a parent (measured directly, not read off a lint that might not check it). A verb that prevents a failure with 0 occurrences in 101 hand-made opportunities is speculative; its shape is written down above if it ever stops being. NOTE ON MY OWN ALARM: I first read "32 parents with non-empty npt/eht vs 132 with parent-trdd" as evidence of mass orphaning. That was a miscount — 32 FILES naming 103 IDS. The direct count is what settled it, and the wrong reading is recorded because a count of files standing in for a count of references is the same proxy error this card keeps finding.
- [x] **META-MISSING — the set is CLOSED, and that is the explanation.** 154 warns: 80 `assignee`, 68 `created-by`, 6 `min-approval-requirement`. The newest affected card is dated **2026-08-25**, two days before `trddgrep new` landed, and **zero** of the cards created since carry the finding — so this is a bounded historical set that cannot grow, because every new card now goes through a verb that writes all three fields. It cannot be repaired either: `created-by` is unrecoverable, and deriving it from git would give the same human owner for all 68 — data-shaped output carrying no provenance, which is worse than the absent field because it stops the next reader checking. `assignee` on a years-old backburner card would be an assignment nobody made. So the box's "explained in place" is satisfied ONCE, here, with the measurement, rather than by editing 154 files to repeat one sentence.

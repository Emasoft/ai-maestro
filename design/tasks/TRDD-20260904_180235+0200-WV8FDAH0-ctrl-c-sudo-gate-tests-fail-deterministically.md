...
**0/14 without** — Fisher p ≈ **0.11**, suggestive and not significant. And **three of the
four failures sit in one batch that ran under background load**; excluding it, the rest is
1 failure in 22 runs (p ≈ 0.36), i.e. nothing. Whether the file is flaky as it now stands is
open — step 0 on that card. A real regression here could still read as "the flaky one again".

...
## Acceptance
- [ ] move accepts complete|completed → superseded on an ARCHIVED card when --superseded-by resolves to any existing card (open or terminal) and --reason names the checklist gap; REFUSES when the source is published, live or failed (a release-pipeline statement — R-Y makes force-supersede NON-EXEMPT)

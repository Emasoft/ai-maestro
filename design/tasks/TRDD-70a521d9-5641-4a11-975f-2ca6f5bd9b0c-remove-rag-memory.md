...
|---|------|----------|------------|
| 1 | `lib/agent-startup.ts::discoverAgentDatabases()` scans FS for `agent.db` files — after removal returns empty, **silent regression** (no subconscious processes start). | **HIGH** | Rewrite to read from `registry.json` BEFORE touching CozoDB (Phase 0). |

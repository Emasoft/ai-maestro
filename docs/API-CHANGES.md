...
| `scripts/amp-helper.sh` (LOCAL delivery) | **LOCAL AMP delivery is now Ed25519-verified**, matching the server `/route` path — `save_to_inbox` no longer implicit-trusts an `aimaestro.local` sender; it resolves the sender's **registered** pubkey (name→UUID `.index.json` Path 1b) and verifies the signature fail-closed. Plus: `sign_message` refuses a group/other-readable private key; explicit `AMP_HOST=1` host identity; install-time orphan-dir GC (RULE-0 move-to-backup, fail-closed) (TRDD-5KKO25RO). | CLI-only. A forged/unsigned local message → delivered UNTRUSTED, never `sig_valid=true`. `install-messaging.sh -y` migrates private keys to 0600 + prunes unindexed agent dirs. |

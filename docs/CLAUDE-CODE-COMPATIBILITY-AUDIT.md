...
`claude --version` reports **2.1.240**; nothing here is speculative. **9 releases audited, 2
APPLIED — both the same defect, both fixed in this pass.** Run as five parallel audits
(`reports/cc-align/u{1..5}-*.md`, gitignored, janitor-archived after 48 h), every unit re-verified
by the coordinator against the tree; the ledger with the commands is
`reports/cc-align/DELEGATION.md`.

*(The five reports self-report 29+44+49+40+44 = 206 entries. That figure is a SUM OF SELF-REPORTS,
not a count anyone took against the changelog: a worker that silently skipped entries would shrink
the true denominator invisibly. The per-entry verdicts below were each verified; the total was not,
so it is stated here as provenance rather than as a measurement.)* **The unaudited gap 2.1.225–2.1.231 is recorded under Remaining
below — this pass covers only the range that was supplied.**

| Entry | Verdict | Notes |
|---|---|---|
| **2.1.233 — todo tools (`TaskCreate`/`Get`/`Update`/`List`, `TodoWrite`) removed by default on Opus 4.8, Sonnet 5, Fable 5, Mythos 5+; `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` restores** | **APPLIED ×2** | The only real breakage in the whole range. Two files instructed an agent to CALL a tool that no longer exists by default, and `CLAUDE_CODE_ENABLE_TODO_TOOLS` is set **nowhere** — measured 0 at every writable source, not just the tracked tree: `~/.claude/settings.json`, `ecosystem.config.js`, `.env.local`, untracked files in the repo, and `lib/claude-settings-enforcer.ts` (which is what stamps env into every managed agent's settings). Including the **live server process env** (`ps eww -p 4054` → 0, against a control of 153 readable `KEY=` pairs proving the dump is non-empty). An earlier version of this line claimed that check was merely "unconfirmed" — in fact my first PID lookup had matched a different `ai-maestro`-containing process line, so I did not know whether it had resolved at all. Re-run against the real server pid with a control. Fixed **tool-free rather than by exporting the flag** — reviving a withdrawn surface fleet-wide to keep two sentences literally true is the wrong direction. (a) `.claude/project/memory/trdd-conventions.md:41` "uses a TaskCreate entry" → states the outcome, correction demoted to lesson `[^8]`. (b) `.claude/agents/scenario-improvement-implementer.md:65` "Emit a TodoWrite list…" → "Work them in `priority:` ascending order"; that agent pins `model: opus[1m]`, one of the four named models. **12 files match the tool names; only these 2 are mandates** — the rest are ai-maestro's own `TaskCreateForm`/`TaskKanbanBoard`/`TaskPanel` React components, `github-project.ts`'s `CachedTaskList`, `converter/types.ts`'s `TaskCreated` events, `converter/rewrite/tools.ts:25`'s `'TodoWrite': 'todo_write'` mapping (must stay — historical transcripts still convert through it), and transcript analysis that remains true of the transcripts it measured. |

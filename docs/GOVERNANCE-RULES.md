...
| R17.24 | **An agent may use only a WHITELIST of the plugins enabled at user scope** (`USER_SCOPE_PLUGINS_ALLOWED_FOR_AGENTS` in `lib/ecosystem-constants.ts`, mirrored byte-for-byte in `scripts/ecosystem-config.sh`; USER-named 2026-08-27: janitor, perfect-skill-suggester, claude-plugins-validation, llm-externalizer, visual-communicator). The harness enforces it by writing `"<plugin>": false` into the agent's OWN `.claude/settings.local.json` for every user-scope plugin not on the list — a local `false` overrides the user-scope `true` for that process alone, which is the ONLY lever Claude Code offers (no per-plugin allowlist key exists; `--plugin-dir` does not exclude user-scope plugins). It runs inside `wakeAgent` beside the R17 core heal (`lib/user-scope-plugin-whitelist.ts`), so every agent is covered on its next start — NEVER as a loop (R17.18) and NEVER by writing `~/.claude/settings.json` (the IRON no-user-scope-writes rule): the host user's other plugins are theirs. Whitelisted keys are not written at all, so an operator may still disable one per agent by hand. An UNREADABLE user-scope file refuses the wake (fail closed): an agent with an unknown plugin surface must not start. Adding a name to the whitelist changes this rule → `manager` | Explicit |

...
changelog:
  - "5.5.1: R6.6 + R6.9 prose corrected to match the code (spec led: governance-spec 2.6.0 -> 2.6.1; this row is the emanation; TRDD-2XV78BND, MANAGER-approved 2026-08-15). R6.6 drops 'unconditional' — the human sender's outbound Y is conditional on the sender context resolving (the R38.2 fail-closed deny, test-pinned); the old wording licensed deleting that guard. R6.9 names the generic auth gate (amp-service 401) as today's enforcement and records the dedicated isSubagent guard as LATENT — no production caller passes isSubagent: true. PATCH: record-only, no behavior change, no enforcement-map row change."
  - "5.5.0: R42.9 CORRECTED by direct USER directive, same day as its addition (spec led: governance-spec 2.5.0 -> 2.6.0; this row is the emanation). The outbound half is INVERTED: writing permissions.deny SendMessage into an agent workdir is now FORBIDDEN — the deny keys on the whole client tool and breaks the agent's own subagent handling (USER: 'SendMessage is needed to handle subagents! only crossSessionInbound refuse must be added'). The amp-only-messaging invariant now writes crossSessionInbound refuse ONLY and REMOVES the SendMessage deny it previously planted in live workdirs. Recorded as MINOR with this note (a breaking change to a 12-hour-old rule whose only pinner is the invariant corrected in the same commit)."
  - "5.4.0: NEW SUB-RULE R42.9 — CLIENT-NATIVE CROSS-SESSION MESSAGING DENIED IN HARNESS WORKDIRS (USER, 2026-08-20, direct mid-turn directive; TRDD-027HZOYN). Authored in design/specs/governance-spec.md FIRST (spec-version 2.4.3 -> 2.5.0) per the authority inversion; this catalog row is its emanation. Both directions: permissions.deny SendMessage (outbound, shipped 556f340f) + crossSessionInbound refuse (inbound), forcefully maintained in each agent workdir settings.local.json by the amp-only-messaging invariant, self-repairing on create/wake/periodic. EXPLICIT carve-out: a session own sub-agents are untouched — the rule binds cross-session edges only, enforcing R42.3. MINOR: a new rule, no existing MUST changed."
  - "5.3.3: R42.8 exception-verb list corrected to name `block-state` (found by the MANAGER plugin session building its incorporation, reported 2026-08-08; same failure family as 5.3.1, mirrored — there the doc named verbs that 403, here it omitted one the procedure needs). The row said `read-prompt` and `answer` ONLY while the deployed CLI help and the server both permit `block-state` cross-agent under the SAME `unblock-prompt` action (lib/sudo-guard.ts routes GET /api/agents/[id]/block-state there). The omission was not cosmetic: the hook's chat-state carried AskUserQuestion in 0/419 surveyed files, so a caller obeying the ONLY literally reads null from read-prompt and the one prompt shape that blocks an agent indefinitely is invisible — dropping block-state from DETECT would silently defeat the unattended operation R42.8 exists to enable. block-state carries no caller decision (a read, exactly like read-prompt), so the inject/slash/queue exclusion rationale does not touch it. PATCH (names the ratified implementation; no new permitted behavior)."
  - "5.3.2: R39.2 PUBLICATION FACT re-aligned to the spec (ai-maestro#86 doc correction). The row still said the ASSISTANT plugin is 'intentionally NOT a published GitHub repo' — stale since 2026-07-22: Emasoft/ai-maestro-assistant-role-agent is PUBLIC (verified via gh 2026-08-06, pushed 2026-07-23) and listed in the ai-maestro-plugins marketplace manifest. governance-spec.md R39.2 was already corrected; under the authority inversion this file is the emanation and must follow. It remains absent from PREDEFINED_ROLE_PLUGIN_NAMES — an OPEN QUESTION on ai-maestro#86 F2 (consumers assume exactly 8; never 'fix' the count to 9). Mirror-sync: design/specs/role-plugins-spec.md RP-ASSISTANT-01 (1.0.0 -> 1.0.1). PATCH — factual correction, no rule-semantics change."
  - "5.3.1: R42.8 CORRECTED AGAINST ITS OWN IMPLEMENTATION (TRDD-AODXPI5E), spec 2.4.0 -> 2.4.1. Two defects, both found by BUILDING the enforcement rather than by re-reading the text. (1) The clause named `inject` and `queue` as exception verbs and (f) advised 'prefer queue over interrupting'. Both are wrong: those verbs deliver an arbitrary command (queue at the next idle window), so they express the CALLER's decision — R42.1 exactly — and remain SELF-ONLY for every title. A MANAGER following the rule as written would have been 403'd by the server on advice the rule itself gave. The exception is `read-prompt` and `answer`, nothing else. (2) (e) now states WHY no agent may answer an identity-vouching prompt: the ai-maestro SERVER is the sole notary of identity — it created or imported every agent, registered it and its AID in the signed ledger, alone holds the key that signs and rotates that AID, and alone signs and verifies AMP messages. Identity is ESTABLISHED by the server's verification, never ASSERTED by a party to the exchange; that is also what makes (c)'s title scoping meaningful, since authorize() reads back the server's notarized record rather than a caller's claim. R42.8 is now ENFORCED (was UNENFORCED): lib/authorization.ts carries the 'unblock-prompt' action and its matrix, services/agents-core-service.ts carries Gate 0b (the target must actually be blocked), lib/sudo-guard.ts routes POST /api/agents/[id]/prompt/answer to the new action — that last one silently, since the guard runs BEFORE the handler and leaving it on 'send-command' would have denied every cross-agent unblock while both lower layers claimed to permit it. PATCH (correction + enforcement of an existing rule; no new permitted behavior)."
  - "5.3.0: NEW SUB-RULE R42.8 — BLOCKED-PROMPT UNBLOCK (USER, 2026-08-05, direct first-person grant; ai-maestro#125, TRDD-AODXPI5E): a MANAGER or CHIEF-OF-STAFF may READ and ANSWER a pending permission/AskUserQuestion prompt that is BLOCKING another agent, in realtime, via the frozen aimaestro-session.sh. Authored in design/specs/governance-spec.md FIRST as GOV-R42's new clause (spec-version 2.3.0 -> 2.4.0) per the v4.8.0 authority inversion; this catalog entry is its emanation. It was needed because the capability was BUILT, SHIPPED and TITLE-GATED while R42.2's 'no title is exempt' told agents it did not exist for them — so on 2026-08-05 a MANAGER holding the authority, the AID and the CLI refused TWICE to unblock a stalled AUTONOMOUS agent and escalated to the human, defeating the unattended operation the product exists to provide. R42.0/R42.1/R42.2 are re-scoped from 'influence' to 'assign, redirect, or perform another agent's WORK', which is what they always meant to protect; the comm graph is untouched, because an unblock answers a prompt the agent itself raised and can express nothing else. Eight constraints carry the grant: (a) blocked-only trigger; (b) unblock never drive — answer ONLY the pending prompt; (c) title-scoped and exhaustive — MANAGER any agent but ASSISTANT, COS its OWN TEAM only; (d) never an ASSISTANT under any title (its session is the surface a human talks through, so injected text launders an agent instruction into apparent human intent); (e) identity-vouching prompts ESCALATE, never self-answered; (f) read-prompt before answer; (g) server-enforced by AID_AUTH + title, failing closed; (h) audited in the agent ops ledger. MINOR (new sub-rule, new permitted behavior)."
  - "5.2.0: NEW SUB-RULE R42.7 — DAEMON FLEET RESTART (USER, delegated 2026-07-30, TRDD-QZL828OD): the ai-maestro server acting as the absorbed janitor daemon may restart harness agents on its own host after a global change it just applied (an ai-maestro-plugins plugin update, or a ~/.claude/settings.json runtime-env re-apply). Authored in design/specs/governance-spec.md FIRST as GOV-R42's new clause (spec-version 2.2.0 -> 2.3.0) per the v4.8.0 authority inversion; this catalog entry is its emanation. It was needed because R42.super revoked the cross-agent case of POST /api/sessions/[id]/restart, so no actor could restart the fleet unattended — and the restart-notifier seam in services/auto-update-service.ts had been DEAD since it was written (server.mjs passed no notifier, and the design it documented routed through the UI's useRestartQueue, which needs an open browser: useless for the unattended host that most needs the update). Six constraints carry the grant and are implemented, not assumed: (a) uniform fan-out, never a chosen agent; (b) zero content — stored args replayed, never a keystroke; (c) the same idle_prompt + subagent gate the human's button obeys, force hard-coded false; (d) same-host, harness-only, via the ONE workdir authority; (e) every restart audited as ledger op fleet_restart; (f) no route/script/CLI reaches it — pinned by a source scan, because who CAN call it is not observable from any test of the function itself."
  ...
  - "4.6.0: REFINED the ASSISTANT authority model (USER ruling, 2026-07-22) — MINOR, new behavioral clauses on R39. The USER detailed how the MANAGER+AUTONOMOUS ASSISTANT must actually behave, which REVISES R39.5/R39.7 and ADDS R39.8/R39.9. (1) R39.8 — the ASSISTANT carries NONE of the MANAGER's approve-other-agents machinery (instructions or scripts): it approves ONLY its OWN TRDDs (self-mandates, Tier 0 — it never asks the MANAGER to approve them) and never approves/commands/sends directives to any other agent. (2) R39.9 — the MANAGER is the SOLE agent that may reach the ASSISTANT, and only to ASSIGN it a TRDD (never to configure it — config is USER-only via the UI, R39.4); the ASSISTANT accepts such a task ONLY if its bound USER has approved MANAGER-collaboration, and may REFUSE any assigned task (never a forced mandate target, R41 holds); on a SHARED GitHub project it collaborates as a PEER with EQUAL authority, subordinate only to its own USER — its latitude is deliberate (the USER is free, and the ASSISTANT must be free to follow). (3) R39.5 REVISED — 'messages only its own user' → 'messages only its own user AND the MANAGER' (the single agent channel, R39.9). (4) R39.7 REVISED — 'invisible to other agents' → 'invisible to other agents EXCEPT the MANAGER'. NOT touched: 'obeys ONLY its user', 'never a mandate target (R41)', 'no MANAGER/COS/MAESTRO approval to act for its user' — the MANAGER may ASK (USER-gated, refusable), never COMMAND. PENDING (not in this commit — the R39.1-R39.4 surface build stays HELD, transition phase): the PLUGIN rewrite that strips the inherited approval machinery (a LOCAL/D4 source at ~/agents/role-plugins/roles-marketplace/ — outside this repo), and the comm-graph ENFORCEMENT of the MANAGER<->ASSISTANT edge (lib/communication-graph.ts currently permits only the user edge — safely stricter than the rule). FUTURE (noted, not scheduled): give the ASSISTANT MAINTAINER-style GitHub-repo skills — and the AUTONOMOUS role-plugin has long lacked GitHub-repo skills too and should be improved. Full spec + the plugin-rewrite strip-list: TRDD-W9FA6ACZ. No code changed in this commit; `@spec:comm-graph` is R6's 8-title matrix (assistant not a node in it), so conformance stays green."
  - "4.5.1: CORRECTED R39.2's ASSISTANT composition back to MANAGER + AUTONOMOUS (USER re-ruling, 2026-07-22). The 4.4.0 (2026-07-16) revision to 'MANAGER + MAINTAINER' was the error: MAINTAINER is a repo-bound issue-triage/publish role, whereas the ASSISTANT plans like a MANAGER (it listens to its bound user) and codes like an AUTONOMOUS agent (independently, no team, not directed by the MANAGER — consistent with R39.5). This realigns R39.2 with the sources that were ALREADY correct: the built LOCAL/D4 plugin persona (~/agents/role-plugins/roles-marketplace/ai-maestro-assistant-role-agent, built 2026-06-19) and the lib/ecosystem-constants.ts:280-288 comment, both MANAGER+AUTONOMOUS. Also dropped R39.2's stale '(still to be created)' — the plugin EXISTS (LOCAL/D4, correctly absent from PREDEFINED_ROLE_PLUGIN_NAMES; the Emasoft GitHub 404 is by design). No code change (types/agent.ts + lib/communication-graph.ts already carry 'assistant'; the composition is persona prose, not enforced logic). Mirror-sync: design/specs/governance-spec.md R39.2, the r39 project-memory note (its stale '+ MAESTRO' obedience clause also fixed to 'not even the MAESTRO' per R39.5), and TRDD-W9FA6ACZ STATE. Resolves the composition-drift the USER escalated on TRDD-W9FA6ACZ; the R39.1-R39.4 surface build stays HELD (transition phase). PATCH — a rule-text correction, no behavior change."
  - "4.5.0: Added R49 (CRITICAL — IRON, USER-set, 2026-07-16) — the fleet REFUSAL PROTOCOL, closing Emasoft/ai-maestro#71. An approver is a GUIDE, not a GATE: a refusal is the START of the work on a proposal, not the end (the refusal half of R41's APPROVAL protocol). R49.1 — a valid refusal MUST name (a) the precise defect (the exact command/input/abuse/rule, never 'insufficiently secure'), (b) the bar for acceptance, (c) an explicit invitation to re-propose; a bare 'no' names no defect and is itself a defect. R49.2 — refuse the implementation, never the need: measure a refusal by what the proposer does NEXT, since a verdict correct on the merits that ends with the need abandoned is a FAILED refusal. R49.3 — the from-DRAFT corollary (binds the proposer): a refusal that names no defect does NOT authorize stripping/deleting the dependent or derived work — the need stands until a defect is named — and it attaches from the moment a proposal is DRAFTED (never pre-concede destruction in the ask itself, e.g. 'implement X or I strip X'), asking before destroying anything if scope is unclear (RULE-0 for capabilities). R49.4 — the MESSAGE is the channel, the tool is the paperwork: a decision is DELIVERED as a message carrying arguments+explanations with the approver in-thread through the revision rounds, and column:refused + an Approval-log line only RECORDS the outcome; where no AMP thread exists (a plugin session <-> the MANAGER) the cross-repo GitHub issue IS the channel. R49.5 — iterate (several refine-and-re-propose rounds are the process working); binds every authority (MANAGER T2, COS/ORCH T1) and the agent when it is the one refused. R49.6 — the refusal AND its named defect are recorded on the governing GitHub issue and/or the TRDD ## Approval log. Operating detail EXPANDED in the DEP overlay rules/aimaestro/aimaestro-trdd-approval.md (Part B). From the incident where the ai-maestro hub correctly refused CORE's scripts and CORE began deleting its own working skills to make the 'no' go away — caught by the USER; a correct refusal and a destructive one look identical in the log, so the duty attaches to every refusal."
...
| R17.17 | The `ai-maestro-plugin` **MUST NOT be installed at user scope** (`--scope user`). It MUST only exist at local scope in each agent's working directory. If the AI Maestro server detects the plugin enabled at user scope (`~/.claude/settings.json` — **not** `settings.local.json`, which is a PROJECT-scoped override that nothing reads at the user-home level, so a guard writing there would log success and enforce nothing), it MUST disable it at user scope on startup. User-scope installation would make the plugin load in ALL Claude Code projects on the host, not just AI Maestro agents | Explicit |
...
| R17.24 | **An agent may use only a WHITELIST of the plugins enabled at user scope.** The whitelist is a CONFIGURABLE SETTING — `agentPluginWhitelist` in `~/.aimaestro/system-settings.json`, edited from the dashboard (Settings → Extensions → Plugins, the "Agents" column) through the strict route `PATCH /api/settings/agent-plugin-whitelist`, read by `lib/agent-plugin-whitelist-store.ts`. `DEFAULT_USER_SCOPE_PLUGINS_ALLOWED_FOR_AGENTS` in `lib/ecosystem-constants.ts` (mirrored in `scripts/ecosystem-config.sh`) supplies the DEFAULTS — USER-named 2026-08-27: janitor, perfect-skill-suggester, claude-plugins-validation, llm-externalizer, visual-communicator — and applies ONLY when the key is absent; a present list is honoured verbatim, EVEN IF EMPTY. Nothing may hard-code the list's length or read the constant in place of the store. The harness enforces it by writing `"<plugin>": false` into the agent's OWN `.claude/settings.local.json` for every user-scope plugin not on the list — a local `false` overrides the user-scope `true` for that process alone, which is the ONLY lever Claude Code offers (no per-plugin allowlist key exists; `--plugin-dir` does not exclude user-scope plugins). It runs inside `wakeAgent` beside the R17 core heal (`lib/user-scope-plugin-whitelist.ts`), so every agent is covered on its next start — NEVER as a loop (R17.18) and NEVER by writing `~/.claude/settings.json` (the IRON no-user-scope-writes rule): the host user's other plugins are theirs. Whitelisted keys are not written at all, so an operator may still disable one per agent by hand. An UNREADABLE user-scope file — or an unreadable whitelist store — refuses the wake (fail closed): an agent with an unknown plugin surface must not start, and a corrupt store must never be masked as the defaults. Editing the LIST is an operator action through the sudo-gated dashboard route; changing the DEFAULTS or this rule's mechanism → `manager`. **CONSUMER CONTRACT (TRDD-BD8OS8U7):** because the whitelist is enforced as a per-agent LOCAL override, any tool that enumerates the plugins ENABLED FOR AN AGENT (a skill/agent suggester, an element index, a plugin-surface audit) MUST read that agent's `.claude/settings.local.json` at USE time, honouring Claude Code's precedence (local beats user) — never only `~/.claude/settings.json`, and never at index-build time alone: a user-scope read sees every plugin the host has, including the ones this rule switched off for that agent (measured 2026-08-27: perfect-skill-suggester's index-time filter suggested elements from R17.24-disabled plugins). | Explicit |

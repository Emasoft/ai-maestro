...
function getRegisteredAgents() {
  ...
  try {
    ...
    // Validate each entry has a well-formed string id
    ...
    return agents.filter((entry) => {
      if (
        ...
        validIdPattern.test(entry.id)
      ) {
        return true
      }
      console.warn('[DB-SYNC] Skipping invalid registry entry (missing or malformed id):', JSON.stringify(entry))
      return false
    })
  } catch (error) {
  ...
  try {
    ...
    const cozoDbModule = await import('../.next/server/app/api/agents/[id]/database/route.js')
      ...
      .catch(async () => {
        ...
        const aiMaestroDir = statePath('agents', agentId)
...
export async function syncAgentDatabases() {
  ...
  for (const agent of agents) {
    ...
    try {
      ...
      // A single malformed agent ID must not crash the entire sync loop
      console.error(`[DB-SYNC] ❌ Error processing agent ${agentId}:`, error.message)
      results.failed++
      results.agents.push({
        id: agentId,
        hasDb: false,

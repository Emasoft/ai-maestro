...
/**
 * CORS allowance for the Analytics health PROBE only. AnalyticsSection.tsx runs on ai-maestro's
 * main port and, BEFORE it frames the dashboard, fetches this proxy (main port + 1) to tell
 * "running" from "not running". That fetch is cross-ORIGIN — the port differs — so without an
 * explicit Access-Control-Allow-Origin the browser discards even a 200 and the panel falsely
 * shows "AgentlensPro isn't running" while the server is perfectly healthy.
 *
 * We echo the ai-maestro origin and ONLY it — derived the SAME way frameAncestorsFor derives the
 * trusted parent (same Host, pinned main port). A mismatched origin gets nothing, so this widens
 * no trust: by the same argument the frame-policy comment makes, the caller is already past the
 * IP filter + session gate and could read the dashboard directly; CORS only lets its own page
 * READ a health ping it is already allowed to make. Never `*`, and paired with Allow-Credentials
 * because the probe sends the session cookie.
 */
export function corsHeadersFor(originHeader, hostHeader, mainPort) {
  const origin = String(originHeader || '')
  if (!origin) return {}
  const host = String(hostHeader || '').replace(/:\d+$/, '')
  if (!host) return {}
  const allowed = new Set([`http://${host}:${mainPort}`, `https://${host}:${mainPort}`])
...
function deny(res, code, message, extraHeaders = {}) {
  res.writeHead(code, {
...
export function createAnalyticsProxy({ port, mainPort, bindAddress, needsIpFilter, isAllowedSource }) {
  const server = http.createServer(async (req, res) => {
    // CORS for the same-host Analytics health probe (see corsHeadersFor). Computed BEFORE the
    // auth gate so even a 401/503 is readable by the probe — otherwise the browser rejects the
    // error response too and every state collapses to a misleading "isn't running".
    const cors = corsHeadersFor(req.headers.origin, req.headers.host, mainPort)
    ...
    if (!validateSessionCookie(req.headers.cookie)) {
      deny(res, 401, 'Unauthorized — sign in to AI Maestro first.', cors)
      return
    }
    ...
    if (!SAFE_METHODS.has(req.method || '') && !isMaestro) {
      deny(res, 403, 'Forbidden — changing AgentlensPro settings requires the MAESTRO user.', cors)
      return
    }
    ...
    if (viewerKey) {
      ...
      deny(res, 503, `AgentlensPro viewer key unavailable at ${EMBED_KEY_PATH} — cannot assert the viewer role. Restart AgentlensPro (it recreates the key on boot).`, cors)
      return
    }
    ...
    const upstream = http.request(
      ...
      (upRes) => {
        ...
        // Let the same-host probe read this response cross-port (see corsHeadersFor). No-store
        // above makes the missing Vary: Origin moot — nothing is cached to poison.
    ...
    upstream.on('error', (err) => {
      ...
      if (!res.headersSent) {
        deny(res, 503, `AgentlensPro is not reachable on ${UPSTREAM_HOST}:${AGENTLENS_DASHBOARD_PORT} (${err.code || err.message}). Start it with: agentlenspro`, cors)
      } else {
        res.destroy()
      }
    })

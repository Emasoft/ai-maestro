...
export function createAnalyticsProxy({ port, mainPort, bindAddress, needsIpFilter, isAllowedSource }) {
  const server = http.createServer(async (req, res) => {
    ...
    const upstream = http.request(
      {
        ...
        path: req.url,
        headers,
        // Bound a hung upstream so a stalled AgentlensPro cannot pin the client's connection
        // open indefinitely. The dashboard is loopback + serves sub-second local telemetry, so
        // 30 s is far past any legitimate response; on fire we destroy, which routes through the
        // 'error' handler below (503 if nothing was sent yet, else tear the socket down).
        timeout: 30_000,
      },
      (upRes) => {
        const out = {}
    ...
    // The socket-idle timeout does NOT auto-destroy the request — it only fires the event, so we
    // must destroy explicitly; that surfaces through the 'error' handler as a named 503/teardown.
    upstream.on('timeout', () => upstream.destroy(new Error('upstream timeout (30s)')))

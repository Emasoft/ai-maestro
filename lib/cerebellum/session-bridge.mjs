...
class TerminalOutputBuffer {
  ...
  write(data) {
    ...
    for (const listener of this.listeners) {
      try { listener(data) } catch (err) { console.error('[TerminalOutputBuffer] Listener error:', err) }
    }
  }
...
export function getOrCreateBuffer(sessionName) {
  if (!sessionName || typeof sessionName !== 'string') throw new Error('Invalid sessionName')
...
export function removeBuffer(sessionName) {
  const buffer = buffers.get(sessionName)

...
class HostHintBroadcaster {
  ...
  broadcastAll(type) {
    ...
    // Snapshot to a static array before iterating — if any listener
    // synchronously calls subscribe() or unsubscribe(), the Map won't
    // be mutated mid-iteration (which would throw a TypeError).
    const entries = [...this.listeners.entries()]
    for (const [agentId, listener] of entries) {
      const hint = {
        type,
        agentId,
        timestamp

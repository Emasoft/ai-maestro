...
import os from 'os'
import { statePath } from './ecosystem-state-paths.mjs'
...
/**
 * Atomic JSON file write using rename pattern.
 * Writes to a temp file in the same directory, then renames (POSIX atomic).
 * Prevents TOCTOU races when multiple callers do read-modify-write.
 */
...
function validateHosts(hosts) {
  ...
  // Detect if any host IDs were migrated (e.g., 'local' → hostname, case normalization)
  const migrated = hosts.some((orig, i) => orig.id !== migratedHosts[i].id)
  if (migrated) {
    console.warn('[hosts-config] Host IDs migrated in-memory — run setOrganization to persist')
  }
...
export function setOrganization(name, setBy) {
  try {
    ...
    // Idempotency: skip write if organization already matches
    if (config.organization === name.toLowerCase()) {
      return { success: true, message: 'Organization already set', organization: config.organization }
    }

    // Check if already set to a DIFFERENT value
    ...
    // Save config atomically (prevents TOCTOU race with concurrent callers)
    atomicWriteJsonSync(HOSTS_CONFIG_PATH, config)
    clearHostsCache()
...
export function adoptOrganization(organization, setAt, setBy) {
  try {
    // Validate peer-provided organization name before adopting
    if (!isValidOrganizationName(organization)) {
      return {
        success: false,
        adopted: false,
        error: 'Invalid organization name from peer. Must be 1-63 lowercase characters (letters, numbers, hyphens). Must start with a letter and cannot start/end with a hyphen.',
      }
    }

...
export function createRemoteProxy(clientWs, sessionName, hostId) {
  // Reject dead sockets early — prevent attaching listeners to a socket that will never send/receive
  if (clientWs.readyState !== WebSocket.OPEN) {
    clientWs.close(1011, 'Socket not open')
    return
  }

  // SECURITY: SF-058 — no auth check here, Phase 2 will add mandatory auth
  ...
  // Buffer messages received before worker connection is open
  const pendingMessages = []

  // Worker connection opened
  workerWs.on('open', () => {
    isConnected = true
    // Flush buffered messages that arrived before the worker connection opened
    ...
    // Heartbeat: ping both ends every 30s to detect dead connections
    pingInterval = setInterval(() => {
      if (workerWs.readyState === WebSocket.OPEN) workerWs.ping()
      if (clientWs.readyState === WebSocket.OPEN) clientWs.ping()
    }, 30000)
  })

  // Heartbeat interval handle — set inside workerWs 'open', cleared on close
  let pingInterval = null

  // Proxy: Client → Worker (buffer if worker not yet connected)

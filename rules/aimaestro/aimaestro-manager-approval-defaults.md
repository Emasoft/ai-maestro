...
# ai-maestro overlay — Exempt operations, the MANAGER-approval bypass list

> **DEP overlay — installed by the ai-maestro server** into each
> registered agent workdir's `.claude/rules/`. It EXPANDS the IND base
> `trdd-design-tasks.md` (the TRDD format + folder lifecycle, shipped
> globally by the ai-maestro-janitor and assumed present) and pairs
> with the sibling overlay `aimaestro-trdd-approval.md` (the tier
> ladder + transition authority); base content is NOT restated here.

...
show_help() {
    ...
    echo ""
    echo "Archive (delete) a kanban card. Use for cards that reached a terminal"
    echo "state of the 14-stage TRDD-v2 pipeline (e.g. published, live, failed, or"
    echo "superseded) and no longer need a column on the board."
    echo ""
    echo "Arguments:"
...
# server (curl exit 7) killed it HERE and the "❌ Failed to archive task" branch below never
...
RESPONSE=$(curl -s -w "\n%{http_code}" --connect-timeout 5 --max-time 15 \
    -X DELETE "$API/api/teams/$TEAM_ID/tasks/$TASK_ID") || true

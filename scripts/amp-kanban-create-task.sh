...
show_help() {
    ...
    echo "  --status, -s STATUS        Initial status — one of the 19 TRDD-v2 pipeline stages:"
    echo "                               backburner|approval|design|design_ai_review|design_human_review|"
    echo "                               todo|verify_assumptions|plan|dispatch|dev|testing|ai_review|"
...
# server (curl exit 7) killed it HERE and the "❌ Failed to create task" branch below never
# ran — a bare exit 7 with no diagnostic, for a task that was never created. curl still

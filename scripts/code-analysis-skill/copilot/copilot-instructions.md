# Code analysis with tldr + fastedit (GitHub Copilot custom instructions)

Token-efficient code analysis (READ via `tldr`) and AST-scoped editing (WRITE via
`fastedit`) for 18 languages. Reach for these tools BEFORE reading whole files or
editing unfamiliar code: `tldr` extracts ONLY the lines that matter (definitions,
callers, program slices, call graphs, taint/security flows, metrics, dead code,
BM25 + semantic search); `fastedit` edits by symbol so you write only the change,
never the old code repeated back. Invoke them INTENTIONALLY — you choose the query.

# tldr + fastedit — code-analysis rules (KiloCode)

Use `tldr` (read) and `fastedit` (write) for all code navigation and symbol
editing in this workspace. `tldr` extracts ONLY the relevant lines (definitions,
callers, program slices, call graphs, taint/security flows, metrics, dead code,
BM25 + semantic search) instead of reading whole files; `fastedit` edits by symbol
so you write only the change. Invoke them INTENTIONALLY, by symbol — dramatically
cheaper than dumping source into context. `fastedit` requires `tldr` on PATH.

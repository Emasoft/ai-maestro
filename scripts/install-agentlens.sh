...
#
# VERSION FLOOR: 2.8.0 — the release that carries the locked contract test +
# calibrated windows + attribution feed. If it is not yet on npm (owner-gated
# publish), this installer WARNS and CONTINUES; re-run once 2.8.0 publishes.
#
...
if command -v "$AGENTLENS_NPM_PKG" >/dev/null 2>&1; then
    ...
    if [ -n "$CUR" ] && _ge "$CUR" "$AGENTLENS_VERSION_FLOOR"; then
        _ok "$AGENTLENS_NPM_PKG $CUR already installed (>= $AGENTLENS_VERSION_FLOOR) — nothing to do."
        exit 0
    fi

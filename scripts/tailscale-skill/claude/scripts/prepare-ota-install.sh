...
# This script belongs to one specific iOS project and operates on $ROOT_DIR --
# three levels above itself. Bundled into a different skill, that path resolves
# somewhere else entirely (inside this skill it lands on the Claude config dir),
# where the "rm -rf $OUTPUT_DIR" below would run against the wrong tree. Refuse
# unless the project this script was written for is actually here.
if [ ! -e "$PROJECT_PATH" ]; then
  echo "error: $PROJECT_PATH not found in $ROOT_DIR" >&2
  echo "       This script only runs inside the iOS project it was written for;" >&2
  echo "       it is bundled here only as a reference for its Tailscale Serve usage." >&2
  exit 1
fi

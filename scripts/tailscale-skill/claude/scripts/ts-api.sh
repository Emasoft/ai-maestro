...
# This script was written for a project that ships a load-env.sh helper. That
# helper is OPTIONAL here: use it when present, otherwise read the credentials
# straight from the environment, so the script still works standalone in this
# skill instead of dying on a path that belongs to a different project.
...
if [[ -f "$_LOAD_ENV" ]]; then
    ...
    validate_env_vars "TAILSCALE_API_KEY" "TAILSCALE_TAILNET"
fi

API_BASE="https://api.tailscale.com/api/v2"

TS_API_KEY="${TAILSCALE_API_KEY:-${TS_API_KEY:-}}"
TS_TAILNET="${TAILSCALE_TAILNET:-${TS_TAILNET:-}}"

# Checked only for commands that actually call the API, so --help still works
# without credentials.
case "${1:-}" in
    -h|--help|help|"") ;;
    *)
        if [[ -z "$TS_API_KEY" || -z "$TS_TAILNET" ]]; then
            echo "ERROR: set TAILSCALE_API_KEY and TAILSCALE_TAILNET in the environment" >&2
            echo "       (or install the load-env.sh helper this script expects)" >&2
            exit 1
        fi
        ;;
esac

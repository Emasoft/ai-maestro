/**
 * Haephestos Publish Service — validate + copy a built role-plugin into the
 * local marketplace.
 *
 * TRDD-1LFRP6GJ: this logic used to live inline in
 * `app/api/agents/creation-helper/publish-plugin/route.ts`, reachable only
 * over HTTP. The Haephestos persona's documented fallback (a bare `curl`
 * with no credential) 401s under `middleware.ts` — RULED (TRDD-1LFRP6GJ):
 * move the lookup off the API rather than mint a credential for the persona
 * session. The pure validation+copy logic is extracted here so it can be
 * invoked directly, in-process, by:
 *
 *   1. `publish-plugin/route.ts` — the HTTP route, now owner-gated
 *      (`enforceSystemOwner`), for the dashboard/`/aim-publish-plugin`.
 *   2. `services/creation-helper-service.ts`'s publish-request poller — the
 *      persona writes `~/agents/haephestos/publish-request.json`; the
 *      poller (already running server-side for the life of the Haephestos
 *      session, alongside the heartbeat watchdog) picks it up, calls this
 *      function directly (no HTTP, no credential needed), and writes
 *      `~/agents/haephestos/publish-response.json`.
 *
 * No behavior changed relative to the pre-extraction route body — this is a
 * pure move, verbatim, so the existing `tests/integration/haephestos-pipeline.test.ts`
 * (which mocks `fs`, `fs/promises`, `@/services/role-plugin-service`,
 * `@/lib/file-lock`, `child_process` — all still imported here) continues to
 * exercise the real logic unchanged.
 */
...
/** Outcome of {@link publishHaephestosPlugin} — mirrors the HTTP route's former response shapes. */
export interface PublishOutcome {
  status: number
  body:
    | { success: true; pluginName: string; pluginDir: string }
...
function validateElementPresence(pluginDir: string, tomlPath: string): string[] {
  ...
  // swallowed here — it propagates to the caller's outer try/catch (→ 500)
...
/**
 * Validate a Haephestos-built plugin directory and, if it passes, copy it
 * into the local role-plugins marketplace and register it. Same gate order
 * (G1 path traversal, G2 quad-identity, G3 compat fields, G2.5 element
 * presence, G4-G6 copy/manifest/refresh) as the pre-extraction route body.
 *
 * Takes a raw (possibly `~`-prefixed, possibly relative) path — same input
 * shape the HTTP route accepted as `body.pluginDir` and the persona's
 * request-file `pluginDir` field carries.
 */
export async function publishHaephestosPlugin(pluginDirInput: string): Promise<PublishOutcome> {
  // ── G1: Path traversal guard — must be inside Haephestos build dir ──
  const resolvedDir = resolve(pluginDirInput.replace(/^~/, HOME))
  if (!resolvedDir.startsWith(HAEPHESTOS_BUILD)) {
    return {
      status: 403,
      body: { error: `pluginDir must be inside ~/agents/haephestos/build/. Got: ${pluginDirInput}` },
    }
  }
  if (!existsSync(resolvedDir)) {
    return { status: 404, body: { error: `Plugin directory does not exist: ${pluginDirInput}` } }
  }

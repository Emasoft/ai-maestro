...
- **Action:** Run `ls ~/.local/bin/amp-*.sh` and check all seven AMP CLI scripts resolve under `~/.local/bin/` (`amp-init`, `amp-inbox`, `amp-read`, `amp-download`, `amp-delete`, and the two scripts each agent is expected to choose on its own after a Chat directive, per Rule 0: `amp-send`, `amp-reply`)

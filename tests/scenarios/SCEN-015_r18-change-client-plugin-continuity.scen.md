...
data_produced:
  - 1 test agent "scen015-r18-test" (temporary, created and deleted)
  - ~/agents/scen015-r18-test/ working directory (temporary, deleted)
...
prerequisites:
  ...
  - No pre-existing agent named "scen015-r18-test"
...
- **Action:** Run `git status` and commit any uncommitted changes with message `pre-scenario: SCEN-015 R18 plugin continuity`
...
- **Action:** Backup config files to `tests/scenarios/state-backups/scen015_<timestamp>/`: `~/.claude/settings.json`, `~/.claude/settings.local.json`, `~/.aimaestro/governance.json`, `~/.aimaestro/agents/registry.json`, `~/.aimaestro/teams/teams.json`, `~/.aimaestro/teams/groups.json`
...
- **Creates:** nothing
- **Modifies:** nothing
- **Verify:** Screenshot saved as SCEN-015/S005-baseline.png
...
- **Verify:** Dashboard loads with agent list. Screenshot: SCEN-015/S006-logged-in.png
...
- **Verify:** Wizard visible, step 1 showing. Screenshot: SCEN-015/S007-wizard-open.png

#### S008: Fill wizard step 1 — agent name
- **Action:** Enter name `scen015-r18-test`, click Next
- **Goal:** Step 2 reached
- **Creates:** nothing (wizard state)
- **Modifies:** nothing
- **Verify:** Wizard advances to step 2. Screenshot: SCEN-015/S008-name-entered.png
...
- **Creates:** nothing
- **Modifies:** nothing
- **Verify:** Claude selected, wizard advances. Screenshot: SCEN-015/S009-claude-selected.png
...
- **Creates:** nothing
- **Modifies:** nothing
- **Verify:** Step 7 confirmation page shown. Screenshot: SCEN-015/S010-confirm-page.png

#### S011: Complete wizard — create agent
- **Action:** Click "Create Agent" button
- **Goal:** Agent is created and appears in sidebar
- **Creates:** 1 agent registry entry, 1 ~/agents/scen015-r18-test/ directory, 1 .claude/settings.local.json with ai-maestro-plugin enabled
- **Modifies:** registry.json
- **Verify:** Agent "scen015-r18-test" appears in sidebar. Screenshot: SCEN-015/S011-agent-created.png

#### S012: Verify core plugin installed (Claude version)
- **Action:** API GET `/api/agents/{scen015-r18-test-id}/local-config` and check that `plugins` array includes `ai-maestro-plugin` with `enabled: true`. Also check on disk: `ls ~/agents/scen015-r18-test/.claude/settings.local.json` and grep for `"ai-maestro-plugin@ai-maestro-plugins": true`.
- **Goal:** Claude version of core plugin is installed and enabled
- **Creates:** nothing (verification)
- **Modifies:** nothing
- **Verify:** Both API and disk confirm the Claude ai-maestro-plugin is enabled. Screenshot: SCEN-015/S012-claude-plugin-installed.png
...
- **Action:** Click agent card "scen015-r18-test" in sidebar
- **Goal:** Profile panel loads
- **Creates:** nothing
- **Modifies:** nothing (UI state)
- **Verify:** Profile panel shows "scen015-r18-test". Screenshot: SCEN-015/S013-profile-open.png
...
- **Creates:** nothing
- **Modifies:** nothing
- **Verify:** Program dropdown visible showing "claude". Screenshot: SCEN-015/S014-config-tab.png

#### S015: Record pre-change snapshot (for R18.2 verification)
- **Action:** Run API GET `/api/agents/{id}/local-config` and save JSON output to `tests/scenarios/reports/SCEN-015_pre_change_snapshot.json`. Count plugins, note role plugin.
- **Goal:** Pre-change plugin list recorded
- **Creates:** 1 JSON file in reports dir
- **Modifies:** nothing (just reading state)
- **Verify:** JSON file exists and lists `ai-maestro-plugin`. Screenshot: SCEN-015/S015-snapshot-recorded.png
...
- **Verify:** API PATCH returns 200, no error. Screenshot: SCEN-015/S016-client-changed.png
...
- **Creates:** nothing (verification)
- **Modifies:** nothing
- **Verify:** File exists with valid YAML content. Screenshot: SCEN-015/S017-universal-ir-exists.png
...
- **Creates:** nothing (verification)
- **Modifies:** nothing
- **Verify:** Directory exists, contains files. Screenshot: SCEN-015/S018-codex-emitted.png
...
- **Creates:** nothing (verification)
- **Modifies:** nothing
- **Verify:** Settings file no longer references the Claude ai-maestro-plugin at the top level. Screenshot: SCEN-015/S019-claude-removed.png
...
- **Creates:** nothing (verification)
- **Modifies:** nothing
- **Verify:** Plugin elements (skills, commands, agents) from the converted Codex plugin are discoverable. Screenshot: SCEN-015/S020-codex-installed.png
...
- **Creates:** nothing (verification)
- **Modifies:** nothing
- **Verify:** `agent.program === "codex"`. Screenshot: SCEN-015/S021-registry-codex.png
...
- **Creates:** nothing (verification)
- **Modifies:** nothing
- **Verify:** Restart indicator present. Screenshot: SCEN-015/S022-restart-needed.png
...
- **Verify:** PATCH returns error with "R18 violation" in message. Registry still shows `program: claude`. Agent `.claude/settings.local.json` still has ai-maestro-plugin. Screenshot: SCEN-015/S023-abort-verified.png
...
- **Action:** Open scen015-r18-test profile panel → Advanced tab → Danger Zone → Delete Agent → check "Also delete agent folder" → type `scen015-r18-test` → Delete Forever
- **Goal:** Agent fully removed from registry and filesystem
- **Removes:** agent registry entry, `~/agents/scen015-r18-test/` directory, tmux session (if any)
- **Verify:** Agent no longer in sidebar, API GET `/api/agents` does not list it. Screenshot: SCEN-015/S024-agent-deleted.png

#### S025: Purge cemetery entry
- **Action:** Navigate to Settings → Cemetery tab → find `scen015-r18-test` entry → Purge
- **Goal:** Cemetery archive cleared
- **Removes:** cemetery entry for the test agent
- **Verify:** Entry no longer in Cemetery tab. Screenshot: SCEN-015/S025-cemetery-purged.png

#### S026: Verify test artifacts removed
- **Action:** Check: no `scen015-r18-test` in registry.json, no `~/agents/scen015-r18-test/` directory, no cemetery entry
- **Goal:** All test-created artifacts are gone
- **Removes:** nothing (verification)
- **Verify:** All three checks pass. Screenshot: SCEN-015/S026-artifacts-gone.png
...
- **Creates:** nothing
- **Modifies:** nothing
- **Verify:** Screenshot saved as SCEN-015/S028-post-test.png, visually matches S005-baseline.png

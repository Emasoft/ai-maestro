...
version: "1.0"
description: >
  PHASE C of 3 (A → B → C, run in order, NO state reset between them). Phase C starts on the LIVE
  fleet and built repo Phase B left behind. The MANAGER has the MAINTAINER cut a v1.0.0 release, then
  ...
  without decompressing. Phase C OWNS CLEANUP for the whole 3-phase run: it deletes all three agents
  and their folders, purges the cemetery, deletes the real GitHub repo + fork + release, and restores
  every config file backed up in Phase A.
client: claude
interhosts: false
device: desktop
subsystems:
...
prerequisites:
  - "PHASE B COMPLETED, and its report read — this phase consumes B's EXIT STATE as its entry contract"
  - The three agents from Phase A are still present and live
  - "`Emasoft/zipsearcher` exists with a feature-complete `main` and green CI"
  - The Phase A state-backup dir (with `MANIFEST.sha256`) is intact for the STATE-WIPE
...
# SCEN-031C — release, verify, and clean up

> **Phase C of 3.** Run order is **A → B → C**. There is **NO state reset between phases** — C starts
> exactly where B ended and does **NOT** re-run setup.
> **Phase C OWNS CLEANUP for the entire run.** Nothing before it deletes anything.

## ENTRY STATE — verify, do NOT create

Read Phase B's report FIRST. Take from it, never guess:
...
- [ ] The Phase A **baseline screenshot path** (S024 compares against it)
- [ ] The Phase A **state-backup dir** path + `MANIFEST.sha256` (S023 restores from it)
- [ ] `Emasoft/zipsearcher` exists with feature-complete `main` and green CI

If the fleet did NOT reach feature-complete in Phase B, **still run Phase C** — the cleanup must happen
regardless of verdict. Record the release/verify steps as BLOCKED with the reason, then clean up fully.
Leaving a live fleet and a real GitHub repo behind is never acceptable.
...
- [ ] Every `rewipe-list` file SHA256-matches the Phase A backup
- [ ] Dashboard visually matches the Phase A baseline screenshot
...
5. **Read fixed inputs ONCE** (this file, Phase B's report). Never re-read.
6. **Screenshot per STEP (Rule 10), not per poll.**
7. **Report your screenshot and poll counts at the end.**

---

## Phase 8: Release + the MANAGER's own smoke test
...
- **Action:** For each (names from Phase B's report): profile → Advanced → Danger Zone → Delete Agent → `aim_sudo_modal` → check "Also delete agent folder" → type the name → Delete Forever.
...
- **Action:** `gh repo delete Emasoft/zipsearcher --yes` (removes the repo, its PRs, and the 1.0.0 release); then delete the AUTONOMOUS's fork by the **full name recorded in Phase B's report** (`gh repo delete <fork> --yes`). Remove the `/tmp` throwaway install from S018.
...
- **Action:** Compare each `rewipe-list` file against the **Phase A** backup; restore any that still differ after the UI deletions.
...
- **Goal:** UI identical to the **Phase A S003** baseline.
- **Creates:** nothing
- **Modifies:** nothing
...
## Phase C END — the whole-run verdict

#### S024z: Write the consolidated verdict
- **Action:** Write the phase report to `reports/scenarios-runner/`, and state the **verdict for the ENTIRE 3-phase run**, not just Phase C. It is a PASS only if: the fleet self-organized from one directive (A), shipped working reviewed software (B), released and verified it (C), **and** never needed a runner nudge at any point. Cite the three phase reports.

...
- The user's pre-existing real agents. **Identify them STRUCTURALLY, never by a hand-kept name list** (see the correction below): a user agent is one whose `workingDirectory` is outside `~/agents/` — in practice `~/Code/*` — **and** which carries **no `governanceTitle` and no role-plugin**, because the user's agents all predate the governance fork. As of 2026-07-14 that is: `apps-svgplayer-development`, `claude-svgskills-writer`, `libs-svg-svgmatrix`, `libs-svg-svgbbox`, `libs-svg-text2path`, `lib-svg-svg2fbf`, `utils-media-smartmediamanager`, `claude-skills-factory`, `tmux-test-audit` — all under `~/Code/*`, all hibernated. Also never touch `~/agents/jvs-*`, `~/agents/swift-*`, `~/agents/my-*`, `~/agents/integrator-rex`.

  > **CORRECTION (2026-07-14, by the USER).** This line previously named `alexandre`, `luckas-bot`, `jhonny-bot`, `jack-bot`, `genny-bot`, `ecos-chief-of-staff-one`, `backend-infrastructure-engineer` (plus `teseo-bot`, `sergei`, `barry`, `default`, which no longer exist) as *"the user's pre-existing real agents"*. **They are not.** Every one of them lives under `~/agents/`, carries a governance title and a role-plugin, and was **created by an earlier scenario run whose cleanup phase was interrupted**. The user has confirmed they are test litter and given standing permission to delete them.
  >
  > **Why the error was dangerous in BOTH directions.** It told every runner to preserve test debris forever (so the litter compounds, run after run), and — worse — it taught the runner to identify user agents *by name*, which silently fails for any user agent not on the list. A name list is a snapshot; the fleet is not. The structural test above cannot go stale, because it reads the two facts that actually define a pre-fork agent. **Never re-introduce a name list as the primary check.**
